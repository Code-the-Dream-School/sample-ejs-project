...
const csrf = async (req,res,next) => {
  ...
  if (!req.session._csrfSecret) {
    ...
    if (!req.session._csrfSecret) { // may have been set by another request
    ...
    if (req.method === "POST") {
      ...
      if (content_type === "application/x-www-form-urlencoded" ||
        content_type === "text/plain" || content_type === "multipart/form-data") {
        let result = false;
        ...
        if (!result) {
          return next(new CsrfError("A POST request was received without a valid CSRF token."));
        }
      }
    }
    next();
